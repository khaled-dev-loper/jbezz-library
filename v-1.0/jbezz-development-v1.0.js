...
let resulater =  {
		...
		fadeIn:function(ms = 100,finish = function(){},start = function(){} ){
			return resulater.run(function(elem){
				$$(elem).animation(function(p){
					...
					$$(elem).css("opacity",p.toString()+"%",true);
					if(p >= 100){
						finish(elem,p);
					}
				},100,ms);
		...
		fadeOut:function(ms = 100,finish = function(){},start = function(){} ){
			return resulater.run(function(elem){
				$$(elem).animation(function(p){
					...
					$$(elem).css("opacity",(100 - p).toString()+"%",true);
					if(p >= 100){
						finish(elem,p);
					}
				},100,ms);
		...
		css:function(key,val,important = false){
			...
			resulater.run(function(elm){
				if(key===true){
					res = window.getComputedStyle(elm).getPropertyValue(val);
				}else{add_style(elm);}
			});
			return res;
		},
		...
		run:function(func){
			if(a !== null){elem.forEach(function(E){func(E)});}else{func(elem[0]);}return resulater;
		}
...
$$.api = function(data){
	...
	//xhttp.onreadystatechange = function() {if (xhttp.readyState == 4){if(xhttp.status !== 200){funcE(xhttp.statusText)}}}
